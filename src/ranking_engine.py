...
def get_cached_model():
    ...
    if _AI_MODEL_CACHE is None:
        ...
        # 3. Load the model - try local first, if not found download and cache it
        if os.path.exists(model_path) and os.path.isdir(model_path) and os.listdir(model_path):
            # Local model exists, load from local path (offline mode)
            _AI_MODEL_CACHE = SentenceTransformer(model_path, local_files_only=True)
        else:
            # Model not found locally, download and save for offline use
